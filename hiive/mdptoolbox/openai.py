...
class OpenAI_MDPToolbox:
    ...
    def __init__(
        self,
        openAI_env_name: str = None,
        openAI_env: gym.Env = None,
        render: bool = False,
        **kwargs
    ):
        ...
        :type rander: boolean
        """
        if openAI_env_name is not None and openAI_env is None:
            raise ValueError(
                "You must provide either an openAI_env_name or an openAI_env"
            )
        ...
        for state in range(self.states):
            for action in range(self.actions):
                for i in range(len(self.transitions[state][action])):
                    ...
                    self.R[state][action] += (
                        tran_prob * self.transitions[state][action][i][2]
                    )
