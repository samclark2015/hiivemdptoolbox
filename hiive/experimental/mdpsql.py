...
def exampleForest(S=3, r1=4, r2=2, p=0.1):
    ...
    with conn:
        ...
        cmd = (
            ...
            INSERT INTO info VALUES('actions', 2);"""
            % S
        )
        c.executescript(cmd)
        cmd = """
...
class MDP(object):
    ...
    def __init__(self, db, discount, epsilon, max_iter, initial_V=0):
        ...
        try:
            ...
            raise ValueError(
                "Cannot determine number of states from "
        ...
        try:
            ...
            raise ValueError(
                "Cannot determine number of actions from "
        ...
        for a in range(1, self.A + 1):
            ...
            try:
                if self._cur.fetchone()[0] != self.S:
                    raise ValueError(
                        "The transition matrix for action %s " "is not stochastic." % a
                    )
            except TypeError:
                raise StandardError(
                    "The check stochastic query for a=%s " "failed." % a
                )
            ...
            if int(row_max) != self.S:
                raise ValueError(
                    "The transition matrix for action %s is "
            ...
            if int(col_max) > row_max:
                raise ValueError(
                    "The transition matrix for action %a id "
...
class ValueIteration(MDP):
    ...
    def __init__(self, db, discount, epsilon=0.01, max_iter=1000, initial_value=0):
