...
class MDPSpec(object):
    def __init__(self, verbose=False):
        ...
        self.states = []
        self.actions = []
        self.state_outcomes: typing.Dict[tuple, typing.List[NextState]] = defaultdict(
            list
        )
        self.reward_outcomes: typing.Dict[tuple, typing.List[Reward]] = defaultdict(
            list
        )
        self.gamma = 1.0
    ...
    def state(self, name=None, index=None, terminal_state=False, extra_data=None):
        ...
        if name not in self.states:
            ...
            new_state = State(
                name, index=index, terminal_state=terminal_state, extra_data=extra_data
            )
    ...
    def action(self, name=None, extra_data=None):
        ...
        if name not in self.actions:
            new_action = Action(
                name=name, index=self.num_actions, extra_data=extra_data
            )
    ...
    def __repr__(self):
        return "Mdp(states=%s, actions=%s, state_outcomes=%s, reward_outcomes=%s)" % (
            self.states,
            self.actions,
            dict(self.state_outcomes),
            dict(self.reward_outcomes),
        )
    ...
    def to_graph(
        self,
        highlight_state: State = None,
        highlight_action: Action = None,
        ...
        for state in self.states:
            if not state.terminal_state:
                for action in self.actions:
                    ...
                    expected_reward = sum(
                        reward * prob for reward, prob in reward_probs
                    )
                    ...
                    if len(next_states) == 1:
                        ...
                        self.set_node_attributes(
                            n=transition,
                            type="transition",  # label=transition_label,
                            fillcolor=color,
                            color=color,
                            style="filled, bold",
                            shape="point",
        ...
        for edge_key, edge_attributes in self._edge_attribute_dictionary.items():
            ...
            if self.verbose:
                print(
                    f"Adding edge: u={u}, v={v}, edges={len(graph.edges)}, attributes={edge_attributes}"
                )
