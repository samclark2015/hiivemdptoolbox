...
class LinearProgramming(object):
    def __init__(self, mdp_spec: mdpviz.MDPSpec):
        ...
        for state in mdp_spec.states:
            if state.terminal_state:
                next_states[state.index, :, state.index] = 1.0

        for (state, action), choices in transitions.rewards.items():
            expected_rewards[state.index, action.index] = sum(
                value * prob for value, prob in choices.items()
            )
    ...
    def compute_q_table(self, max_iterations=100, all_close=None):
        return _fix_point_iterate(
            self.expected_rewards.copy(),
            lambda q_table: self.q_table_from_v_vector(
                self.v_vector_from_q_table(q_table)
            ),
    ...
    def compute_v_vector(self, max_iterations=100, all_close=None):
        return _fix_point_iterate(
            ...
            lambda v_vector: self.v_vector_from_q_table(
                self.q_table_from_v_vector(v_vector)
            ),
            max_iterations=max_iterations,
            all_close=all_close,
        )
    ...
    def q_table_from_v_vector(self, v_vector):
        ...
        q_table = self.expected_rewards + self.gamma * (
            self.next_states * v_vector.reshape((1, 1, self.num_states))
        ).sum(axis=-1)
        return q_table
...
def _fix_point_iterate(initial, iterate, max_iterations, all_close=None):
    ...
    for _ in range(max_iterations):
        ...
        value = next_value
    raise ValueError(
        "No convergence after %s iterations!\n%s" % (max_iterations, value)
    )
