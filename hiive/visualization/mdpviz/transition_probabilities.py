    ...
    def __init__(self, mdp_spec):
        ...
        for state in mdp_spec.states:
            for action in mdp_spec.actions:
                next_states = NextState.get_choices(
                    mdp_spec.state_outcomes[state, action]
                )
                if not state.terminal_state and not next_states:
                    raise ValueError(
                        "No next states specified for non-terminal (%s, %s)!"
                        % (state, action)
                    )
                if state.terminal_state and next_states:
                    raise ValueError(
                        "Next states %s specified for terminal (%s, %s)!"
                ...
                if state.terminal_state and rewards:
                    raise ValueError(
                        "Rewards %s specified for terminal (%s, %s)!"
                        % (next_states, state, action)
                    )
